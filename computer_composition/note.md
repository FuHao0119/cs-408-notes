...
寻址的过程就是根据地址线给定的地址找到对应的存储单元

如果地址线较多，则主存中的地址就会很多，一般可以按照低-高的地址进行划分，也可以使用高-低进行划分。

#### 1. 按字节寻址 (Byte Addressing)

这是现代计算机最常用的方式。

- **规则**：给**每一个小格子**（1 Byte = 8 bit）都编一个独立的门牌号。
- **特点**：地址精度最高。哪怕你只想找内存里的某一个字符，都能直接通过地址定位到它。
- **比喻**：快递柜的**每一个小格**都有自己的编号（0号格、1号格、2号格...）。

#### 2. 按字寻址 (Word Addressing)

这是为了提高读取效率而设计的方式（通常用于指令读取或特定架构）。

- **规则**：给**一组小格子**（1 Word = 机器字长，比如 32位计算机就是 4 Bytes）编一个门牌号。
- **特点**：一次定位一大块。给定一个地址，取出来的是“一整坨”数据（比如直接取出 4 个字节）。
- **比喻**：快递柜是**打包出租**的。每 4 个小格组成一个“大箱子”，你只能按“大箱子”来找（0号大箱、1号大箱...）。
...
| **寻址方式**              | **如果你访问“地址 1”** | **你拿到的是什么？**            | **就像...**     |
| ------------------------- | ---------------------- | ------------------------------- | --------------- |
| **按字节寻址**            | 指向第 2 个格子        | **`[B1]`** (1个字节)            | 拿走第 2 个苹果 |
| **按字寻址**              | 指向第 2 组            | **`[B4][B5][B6][B7]`** (一整组) | 拿走第 2 箱苹果 |
| *(注：假设字长为 4 字节)* |                        |                                 |                 |
...
为什么需要Cache？ 因为需要提升cpu访问主存中的数据的速度

* 局部性原理：

时间局部性：当cpu执行一段程序的时候，程序可能会在极短的时间内访问同一个地址的数据

空间局部性：当cpu访问一个地方的数据的时候，很有可能会在之后接着访问它邻近地址的数据

* Cache行和主存块的映射关系：直接映射
...
```
|     Tag（标记位）    |   Index（索引）   |  Block Offset（块内偏移） |
|---------------------|-----------------|-------------------------|
```



#### 举例说明（32位地址，Cache大小8KB，每块32字节）：

1. **计算参数**：
   - Cache大小 = 8KB = 8192字节
   - 每块大小 = 32字节
   - Cache块数 = 8192 ÷ 32 = 256块
2. **地址划分**：
   - **块内偏移位**：确定块内哪个字节
     - 32字节需要5位（2^5=32）
     - Block Offset = 5位
   - **索引位**：确定放在Cache哪一块
     ...
     - Index = 8位
   - **标记位**：剩下的高位作为标记
     - 总地址32位 - 索引8位 - 偏移5位 = 19位
     - Tag = 19位

text

```
|  Tag(19位) | Index(8位) | Block Offset(5位) |
|-----------|-----------|------------------|
|  高位部分  | 选择Cache块 | 选择块内具体字节  |
```
...
* 负数的反码就是符号位不变，数值位全部取反, 补码 = 反码 + 1

-9 = [反]1,0110   [补]1,0111

优缺点：

<img src="./images/img16.png" alt="image-20260111145320332" />
...
```
(原)x = +1010110 = 0,1010110
(移)x = +1010110+2^7 = 0,1010110 + 10000000 = 1,1010110

y = -1010110 = 1,1010110
(补)y = 1,0101010
(移)y = 1,0101010+2^7 = 1,0101010 + 10000000 = 0,0101010
```
...
* 负数的原码与补码之间互相转换的方法
  ...
  符号位不变，数值位从右往左看，找到第一个 "10" 除了这个"10" 其余的位取反

* 数值位相同的正数的补码与负数的补码快捷转换
...
在C语言中unsigned声明的数据类型都是用*原码*存储的，且第一位不用做符号位，而是当作数值位来用。

在不用unsigned的数据（有符号数）则是用*补码*存储，第一位是符号位。

且char占8位，short 16位，int 32位 long 64位。

* 位数相同的数据类型有无符号数的转换

unsigned char 和 char 都是8位存储，他们之间的数据怎么转换?

直接把8位表示的二进制位按照他们的规则解析，注意无符号数用补码表示。

* 位数不同的数据类型有无符号数之间转换

原数据为无符号整数转到大字长的数据，进行0补齐: 编译器会在原数据位的前面补足够多的零，直到达到位数大的数据类型规定的位数，再按规则进行解析。



原数据位有符号位的整数转到大字长的数据，进行符号位补齐：就是补足够多的原数据符号位。
...
c: 10000001
c -> unsigned short: 11111111 10000001
us = 65409
```
...
x -> (unsigned int): 0000 0000 0000 0000 1111 1111 1111 1010
y = 0000FFFAH
```
...
eg. (short) a: 1111 1111 0011 0011
...
将b0 b5 清0，b2 b3置1
用c语言编程实现（假设寄存器内容保存在变量regData中)
0010 0001: 33 regData & (~33)
0000 1100: 12 regData | 12
...
规则：正数时, 符号位不动，数值为进行左移或右移，超出舍去，空位补0

负数时，原码同整数一样，反码左移右移都是空位补1， 补码左移时低位补0,右移时高位补1



```
假设机器字长8位含1位符号位，真值x=-26, 写出x原码算术左移1位、2位、3位后的形式和对应真值a、b、c,并对结果分析
x(原): 10011010
x<<1 : 10110100
x<<2 : 11101000
x<<3 : 11010000 结果出错

假设机器字长8位含1位符号位，真值x=-26,写出反码算术左移1位、2位、3位后的形式和对应真值a、b、c,并对结果分析
x(反): 11100101
x<<1:  11001011
x<<2:  10010111
x<<3:  10101111  出错

假设机器字长8位含1位符号位，真值x=-26,写出补码算术左移1位、2位、3位后的形式和对应真值a、b、c,并对结果分析
x(补): 11100110
x<<1 : 11001100
x<<2 : 10011000
x<<3 : 10110000  出错

假设机器字长8位含1位符号位，真值x=-26,写出原码算术右移1位、2位后的形式和对应真值a、b,并对结果分析
x(原): 10011010
x>>1 : 10001101
x>>2 : 10000110 精度缺失

假设机器字长8位含1位符号位，真值x=-26,写出反码算术右移1位、2位后的形式和对应真值a、b,并对结果分析
x(反): 11100101
x>>1 : 11110010
x>>2 : 11111001  精度缺失

假设机器字长8位含1位符号位，真值x=-26,写出补码算术右移1位、2位后的形式和对应真值a、b,并对结果分析
x(补): 11100110
x>>1 : 11110011
x>>2 : 11111001 
```
...
补码的算术移位的另一种规则：左移高位移除低位补0, 如果符号位发生变化，则溢出了。右移低位移除，高位补符号。
...
逻辑x>>1 0110 1100
如果是有符号数则11011000是补码 算术x>>1 1110 1100
```
...
将二进制形式的无符号数向左或者向右移动，移出的部分会出现在另一端，形成循环
...
计算机的字长是有限的，因此所能表示的数据范围也是有限的，当运算结果超出所能表示的数据范围时，就会出现溢出(0verflow)



溢出会导致错误的运算结果。

<img src="./images/img20.png" alt="image-20260111145320332" />

如图所示，-5 + -4、 5+4 都会溢出 产生错误的结果。

> 两个数相加的时候只有符号位相同才可能发生溢出

### 溢出检测

溢出检测定点数补码加减运算判断溢出的方法主要有以下3种:
...
<img src="./images/img21.png" alt="image-20260111145320332" />



### 串行进位加法器

<img src="./images/img22.png" alt="image-20260111145320332" />

* 带溢出检测的加法器实现

原理为：检测结果的*数值位进位*和*符号进位*，如果不同说明溢出了，也就是异或运算。

<img src="./images/img23.png" alt="image-20260111145320332" />

* 加法器实现减法操作：

原理： x补  - y补 =  x补 +  (- y)补

而(-y)补，则需要将 y补 全部取反 末位+1

所以要将所有的加数y 与1做异或 这样就取反了

在给第一个加法器的c0赋值为1 这样就末位+1了

<img src="./images/img25.png" alt="image-20260111145320332" />

<img src="./images/img24.png" alt="image-20260111145320332" />

sub = 1时， 就是减法操作， 0就是加法操作

> 串行进位加法器时间性能: (2n+4)T  n表示加法器个数



### 先行进位加法器

先行进位加法器的思想是根据之前所有加法器的两个加数xi,yi组成的两个函数与第一个加法器的进位c0计算所有加法器的前一个进位C_(i-1) 。



这样就不用等待上一个加法器输出的进位Ci了，而是可以直接计算当前的加法产生的值Si



<img src="./images/img26.png" alt="image-20260111145320332" />

根据Gi 与Pi Ci的关系设计新的加法器电路

Si = Pi 异或 Ci

Ci除了第一个输入。其他的都是需要花费时间计算的， C1 C2 C2  本质上都是使用C0算出来的。 所以可以并行计算。

<img src="./images/img27.png" alt="image-20260111145320332" />



* 四个快速加法器联通之后的时间性能推导。

<img src="./images/img28.png" alt="image-20260111145320332" />

这样子设计就是一个串行结构带着四个并行结构， 因为进位信息C是需要等待前一个快速加法器的输出的

但是还有办法一次性计算出所有的C：

组内并行、组间也并行， 他通过一个先行进位电路，将所有加法器提供的G* 和P* 结合C0计算出所有的进位C1 C2 C3

而原来的加法器计算出来的进位信息直接舍弃

<img src="./images/img29.png" alt="image-20260111145320332" />
